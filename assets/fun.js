function getAadhaar() {  //Intialisation function for getting Aadhaar information
  ...
  return new Promise(async (resolve, reject) => {
    ...
    do {  // It will search for devices and services for device sdk ,Fetch on Different Ports from 11100 to 11120
      if ((await getJSON_rdser(port, async function (pidoptions, RDService, portfetch) { //Get device and Pid options from devic
...
let getJSON_rdser = async function (url, V) {
  ...
  device.onreadystatechange = async () => {
    console.log(device.responseText, device.readyState, device.status);
...
let getJSON_info = async function (url, V) {
  let deviceInfo
  var count = 0
...
let getJSONCapture = async function (url, V, wadh, device) {
...
async function checkDeviceStatus() {
  ...
  do {
    console.log(port);
    var result=await getRdServiceInfo(port);
    console.log(result);
    port += 1;
  } while (port < 11120);
}

async function getRdServiceInfo(port) {
  console.log("CALLED");
  ...
  xhr.onreadystatechange = function () {
    ...
    if (status == 200) {
      return { "satus": 200, "response": xhr.response };
      callback(null, xhr.response);
    } else {
      return { "satus": status, "response": xhr.response };
      callback(status);
    }
  };
  xhr.send();
